...
public class uMovie implements java.io.Serializable{
	...
	public static void main(String[] args){
      ...
      }catch(ClassNotFoundException cn)
      {
    	...
    	boolean program = true; //This is to aid with the try catch. If the use enters an invalid response (a letter/word), they will be returned to the menu
    	while (program){

    	
		System.out.println("Welcome to uMovie. Enter '1' if you are a customer, or '2' if you are an administrator");//determines if the user is a customer or admin
		try{
		int x = s.nextInt();
...
//User is a customer:
		...
		if(x == 1){ 
			...
			if (z == 1){ //if the user enters 1, this is to search by release date
			...
			if (z == 2){//if the user enters 2, this is to search by id
			...
			if (z == 3){//if the user enters 3, this is to print out the movies and then return to the home menu
				a.traverse();
				System.out.println("*Returning you to your Home Menu*");
				System.out.println();
			}
			if (z == 4){//this is to access the WishList by entering CC number
				...
				if (wl == 1){//user selects to add to WishList
				...
				if (wl == 2){//user selects to watch next up movie in WishList
					if((d.search(ccnum).getList()) == null){
						System.out.println("There is nothing in your WishList yet!");
						System.out.println();
					}
				...
				if (wl == 3){//User wants to delete a movie from the WishList
				...
				if (wl == 4){//Use wants to go back to the home menu
				}
...
//User is an administator:
		else if(x == 2){ //user entered 2 for admin
			...
			if (q==1){//admin wants to retrieve customer info
			...
			if (q == 2){//admin wants to see least rated and possibly delete it
			...
			int e = s.nextInt();
			while(e< 1 || e > 2){//admin chooses whether or not to delete least rated movie
				System.out.println("Please enter either '1' or '2'");
				e = s.nextInt();
			}
			if (e == 1){//delete leastrated
			...
			//add movie
			if (q==3){//admin wants to add a movie to the database
			...
			//view all movies
			if (q == 4){//admin wants to see all movies in databse
				a.traverse();
				System.out.println("*Returning you to your Home Menu*");
				System.out.println();
			}
			if (q==5){//admin wants to add new customer
				s.nextLine();
			...
			if (q == 6){//admin wants to delete customer from database
				System.out.println("Please enter the last 4 digits of the customer's credit card.");
				int card = s.nextInt();
				if(d.search(card) == null){
					System.out.println("There is not a customer in our database with that credit card on file.");//
			...
			if(q==7){//admin wants to quit program
				working = false;
				program = false;
			}
		}
